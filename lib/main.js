...
module.exports = {
  ...
  activate() {
    ...
    this.disposables = new CompositeDisposable(
      ...
      atom.commands.add("atom-workspace", {
        ...
        "fuzzy-explorer:update": () => this.build(),
      ...
      atom.commands.add(this.selectList.element, {
        ...
        "select-list:update": () => this.update(),
  ...
  globPromise(pattern, itemSet) {
    return new Promise((resolve, reject) => {
      const child = fork(require.resolve("./scan"), { silent: true, env: { ...process.env, ELECTRON_ENABLE_LOGGING: "0", CHROME_LOG_FILE: process.platform === "win32" ? "nul" : "/dev/null" } });
      child.send({ pattern });
